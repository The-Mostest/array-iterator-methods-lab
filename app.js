...
veryOldInventors = inventors.filter((person) => {
    if (person.year < 1599 && person.year > 1500)
        return person;
})
...
isAdultPresent = devs.some((geeza) => {
    return geeza.year >2008;
})
...
isEveryone19OrOlder = devs.every((geeza) => {
    return geeza.year < 2003
})
...
totalYearsLived = inventors.reduce((acc, people) => {
    let lived = people.passed - people.year
    return acc += lived
}, 0)
